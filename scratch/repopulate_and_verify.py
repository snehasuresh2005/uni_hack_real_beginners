...
conn.commit()
print("Reset all 999 product fingerprints & status!")

products = cursor.execute("SELECT id FROM products").fetchall()
print(f"Re-running pipeline across {len(products)} products...")
...
print(f"\nCompleted fresh pipeline run across all {count} products!\n")
