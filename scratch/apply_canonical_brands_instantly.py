...
from backend.pipeline import resolve_brand_and_manufacturer
...
products = cursor.execute("SELECT id, mfg_part_num, part_desc, part_manuf, e1_brand, unilog_brand, dib_brand FROM products").fetchall()
print(f"Applying canonical brand resolution directly across {len(products)} products...")

count = 0
for p in products:
    brand_input = p["e1_brand"] or p["unilog_brand"] or p["dib_brand"] or ""
    norm_mfr, norm_brd, _ = resolve_brand_and_manufacturer(
        p["mfg_part_num"], p["part_desc"], p["part_manuf"], brand_input
    )
    cursor.execute(
        "UPDATE products SET resolved_brand = ?, resolved_manufacturer = ? WHERE id = ?",
        (norm_brd, norm_mfr, p["id"])
    )
    count += 1

conn.commit()
print(f"Successfully applied canonical brand resolution to all {count} products in SQLite database!\n")
